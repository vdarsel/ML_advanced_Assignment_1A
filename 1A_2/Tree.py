    ...
    def Likelihood(self, beta: list[float]):
        if self.isLeaf():
            ...
            for i in range(len(self.cat)):
                ...
                # if the parent node has i for value, the likelihood for (B_1,...B_n) does not depend on other variable
            ...
            for child in self.descendants:
                ...
                # P(children|parent=i) = P(child_1|parent)*P(child_2|parent)*...*P(child_N|parent) independent given parent
            if (self.isRoot()):
